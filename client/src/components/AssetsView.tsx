...
export default function AssetsView({
  ...
  const handleSave = async () => {
    ...
    const selectedCategory = (() => {
      if (selectedAssetType?.parentId) {
        const parent = assetTypesConfig.types.find((type) => type.id === selectedAssetType.parentId)
        if (parent?.label) return parent.label
      }
      const existingCategory = String(editing?.category || '').trim()
      return existingCategory || DEFAULT_ASSET_CATEGORY
    })()
    ...
    return (
      <>
        ...
        <div className="assets-view">
          <PageSkeleton title="Assets" cardCount={3} rowCount={8} />
        </div>
      </>
    )
  }
