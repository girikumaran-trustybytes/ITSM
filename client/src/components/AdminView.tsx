...
type MailboxRouteSeed = Partial<Omit<MailboxRoute, 'smtp' | 'imap'>> & {
  smtp?: Partial<MailboxRoute['smtp']>
  imap?: Partial<MailboxRoute['imap']>
}

const createMailboxRoute = (seed?: MailboxRouteSeed): MailboxRoute => {
                      ...
                      <div className="admin-queue-rules-plain">
                        {leftPanelConfig.ticketQueues.length === 0 ? (
                          ...
                          leftPanelConfig.ticketQueues.reduce((acc, queue) => {
                            ...
                            const existingQueueId = Number(existing?.queueId ?? NaN)
                            if (!existing || (Number.isFinite(queueId) && queueId > 0 && (!Number.isFinite(existingQueueId) || existingQueueId <= 0))) {
