...
function toPermissionEntries(input: unknown): PermissionEntry[] {
  if (!Array.isArray(input)) return []
  return input
    .map((entry) => ({
      permissionKey: String((entry as any)?.permissionKey || '').trim(),
      module: String((entry as any)?.module || '').trim(),
      queue: (entry as any)?.queue == null ? null : String((entry as any)?.queue || '').trim(),
      action: String((entry as any)?.action || '').trim(),
      label: String((entry as any)?.label || '').trim(),
      allowed: Boolean((entry as any)?.allowed),
    }))
    .filter((entry) => entry.permissionKey.length > 0)
}

function toPermissionDraft(input: unknown): Record<string, boolean> {
  const draft: Record<string, boolean> = {}
  for (const permission of toPermissionEntries(input)) {
    draft[permission.permissionKey] = Boolean(permission.allowed)
  }
  return draft
}
...
export default function UserDetailView({
  ...
  const extractActionError = (err: any, fallback: string) => {
    const raw = String(err?.response?.data?.error || err?.message || fallback || '').trim()
  ...
  const isTransientActionError = (err: any) => {
    const lower = extractActionError(err, '').toLowerCase()
    return (
  ...
  const notifyInviteDelivery = async (result: any, sentMessage: string, pendingMessage: string) => {
    ...
    notifyAction('error', copied ? `${pendingMessage} Activation link copied.` : pendingMessage)
  }
        ...
        if (!normalizedUser) {
          try {
            const list = await withTimeout(
              listRbacUsers({ q: String(numericId), limit: 250, principalType: isAgentsMode ? 'agent' : 'all' }),
              7000
            )
    ...
    const loadPermissions = async () => {
      try {
        ...
        const rawPermissions = (snapshot as any)?.permissions
          ...
          || []
        const map = toPermissionDraft(rawPermissions)
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      notifyAction('error', extractActionError(err, 'Failed to save updates'))
    } finally {
      setActionBusy(false)
    }
  }
  ...
  const handleCancelEdit = () => {
    ...
    const rawPermissions = (permissionSnapshot as any)?.permissions
      ...
      || (permissionSnapshot as any)?.data?.permissions
      || []
    const reset = toPermissionDraft(rawPermissions)
  ...
  return (
    <>
      ...
      <div className={`agent-detail-shell${embedded ? ' embedded' : ''}`}>
        {loading && !user ? <PageSkeleton title="User Details" cardCount={2} rowCount={6} /> : null}
