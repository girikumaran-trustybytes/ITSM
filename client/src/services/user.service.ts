...
export async function listUsers(params: { q?: string; limit?: number; role?: string; principalType?: 'user' | 'agent' | 'all' } = {}) {
...
export async function sendUserInvite(id: number) {
  const res = await api.post(`/users/${id}/send-invite`, { requireImmediate: true })
  return res.data
}
