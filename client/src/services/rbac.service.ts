...
export async function listRbacUsers(params: { q?: string; limit?: number; role?: string; principalType?: 'user' | 'agent' | 'all' } = {}) {
  const effectiveParams = { principalType: 'all' as const, ...params }
...
export async function sendUserInvite(userId: number, toEmail?: string) {
