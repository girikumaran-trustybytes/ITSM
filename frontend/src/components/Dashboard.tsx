...
export default function Dashboard() {
  ...
  const maxStatusCount = useMemo(
    () => Math.max(1, ...Object.values(ticketStats.byStatus)),
    [ticketStats.byStatus]
  )

  const priorityTone = (value: any) => {
    const raw = String(value || '').toLowerCase()
    if (raw.includes('critical') || raw === 'p1') return 'critical'
    if (raw.includes('high') || raw === 'p2') return 'high'
    if (raw.includes('medium') || raw === 'p3') return 'medium'
    if (raw.includes('low') || raw === 'p4') return 'low'
    if (raw.includes('overdue') || raw.includes('breach')) return 'critical'
    return 'default'
  }
                ...
                <div className="dash-chart">
                  <div className="stat-list">
                    {Object.entries(ticketStats.byStatus).map(([k, v]) => (
                      <div key={k} className="stat-row stat-row-metric">
                        <span className="stat-name">{k}</span>
                        <div className="stat-track" aria-hidden="true">
                          <span style={{ width: `${Math.max(8, (v / maxStatusCount) * 100)}%` }} />
                        </div>
                ...
                <div className="stat-list">
                  {criticalOverdueList.length === 0 ? <div className="activity-empty">No critical or overdue tickets</div> : criticalOverdueList.map((t) => (
                    <div key={String(t.id || t.ticketId)} className="stat-row">
                      <span className="stat-ticket-title">{String(t.subject || t.ticketId || 'Untitled')}</span>
                      <span className={`priority-pill tone-${priorityTone(t.priority || t.status)}`}>{String(t.priority || t.status || '-')}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {visibleWidgets.topAgents && (
            <div className="dash-panel">
              <div className="panel-head">
                ...
                <div className="dash-chart">
                  <div className="stat-list">
                    {topAgents.length === 0 ? <div className="activity-empty">No assignee data</div> : topAgents.map(([agent, count], index) => (
                      <div key={agent} className="stat-row">
                        <span className="leaderboard-agent">
