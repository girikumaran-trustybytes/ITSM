...
type PrincipalType = 'user' | 'agent' | 'all'

function normalizePrincipalType(input: any): PrincipalType {
  const value = String(input || '').trim().toLowerCase()
  if (!value) return 'all'
  if (value === 'agent' || value === 'staff' || value === 'operator') return 'agent'
...
function buildListScope(
  opts: { q?: string; limit?: number; role?: string; principalType?: string } = {},
  options: { useEndUserFlag?: boolean } = {}
) {
  ...
  if (principalType === 'user') conditions.push(userRoleCondition)
  if (principalType === 'agent') conditions.push(agentRoleCondition)
  if (opts.role) {
...
export async function listUsersLightweight(opts: { q?: string; limit?: number; role?: string; principalType?: string } = {}) {
...
export async function listUsersEmergency(opts: { q?: string; limit?: number; role?: string; principalType?: string } = {}) {
...
export async function listUsers(opts: { q?: string; limit?: number; role?: string; principalType?: string } = {}) {
  await ensureUserCrudSchema()
  const { where, params } = buildListScope(opts)
  const inviteTables = await queryOne<{ modern_exists: string | null }>(
    `SELECT to_regclass('public.invitations')::text AS modern_exists`
  )
