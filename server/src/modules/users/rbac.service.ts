...
import { loadMailConfigFromEnv, sendSmtpMail } from '../../services/mail.integration'
...
let rbacSeedInFlight: Promise<void> | null = null

export async function ensureRbacSeeded() {
  if (rbacSeedReady) return
  if (rbacSeedInFlight) return rbacSeedInFlight
        ...
        await client.query(
          `INSERT INTO permissions (permission_key, permission_name, module, module_name, action, queue, label)
           ...
           SET permission_name = COALESCE(permissions.permission_name, EXCLUDED.permission_name),
               module_name = COALESCE(permissions.module_name, EXCLUDED.module_name),
               label = COALESCE(permissions.label, EXCLUDED.label)`,
          [permissionKey, permissionKey, permission.module, permission.module, permission.action, permission.label]
        )
      }
...
export async function markInvitePending(userId: number, actorUserId?: number) {
  ...
  if (!recipientEmail) throw { status: 400, message: 'User does not have a valid email' }
...
type InviteMailMode = 'invite' | 'reinvite'
...
function invitationContext(overrideAppBaseUrl?: string) {
  ...
  const appBaseMail = String(
    ...
    'support.techdesk@gmail.com'
  ).trim()
  const supportPhone = String(process.env.SUPPORT_PHONE || '').trim()
  const purpose = String(
    process.env.APP_PURPOSE || 'manage tickets, track tasks, and monitor assets'
  ).trim()
...
function resolveInviteFromAddress() {
  const runtimeFrom = String(loadMailConfigFromEnv()?.smtp?.from || '').trim()
  if (runtimeFrom) return runtimeFrom
  const ctx = invitationContext()
  return String(ctx.appBaseMail || ctx.supportEmail || 'support.techdesk@gmail.com').trim()
}

function resolveInviteActivationBaseUrl(overrideBaseUrl?: string) {
...
function isTransientInviteDeliveryError(error: any) {
...
export async function sendUserInvite(
  ...
  options?: {
    ...
    allowPendingOnTransientFailure?: boolean
  }
) {
  const user = await queryOne<{
    id: number
  ...
  const mode: InviteMailMode = options?.mode || 'invite'
  ...
  const insertedInvite = await queryOne<{ invitation_id: number }>(
    `INSERT INTO invitations (
      tenant_id,
      user_id,
      email,
  ...
  try {
    ...
    if (!isTransientInviteDeliveryError(inviteError)) throw inviteError
    ...
    if (!allowPendingOnTransientFailure) {
      const err: any = new Error('Invite email not sent immediately. Please retry.')
...
export async function sendServiceAccountInvite(
  userId: number,
  actorUserId?: number,
  options?: {
    mode?: 'invite' | 'reinvite'
