...
import { getRequiredSecret } from '../../common/auth/secrets'

const ACCESS_EXPIRES = process.env.ACCESS_TOKEN_EXPIRES_IN || '15m'
const REFRESH_EXPIRES_DAYS = Number(process.env.REFRESH_TOKEN_EXPIRES_DAYS || 7)
const ACCESS_SECRET = getRequiredSecret('JWT_ACCESS_SECRET')
const REFRESH_SECRET = getRequiredSecret('JWT_REFRESH_SECRET')
...
async function ensureAuthSchema() {
  if (!AUTH_RUNTIME_SCHEMA_SYNC_ENABLED) return
  if (!authSchemaInit) {
...
export async function warmupAuthSchema() {
  await ensureAuthSchema()
}

export async function warmupAuthDb() {
...
async function verifyUserPassword(inputPassword: string, storedHash: string) {
  ...
  try {
    const row = await queryOne<{ ok: boolean }>('SELECT crypt($1, $2) = $2 AS ok', [plain, hash])
