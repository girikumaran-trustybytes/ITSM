...
type ImapSocket = tls.TLSSocket | net.Socket
...
function extractHeaderValue(lines: string[], name: string): string {
...
async function ensureIngestTable() {
  await query(
    `CREATE TABLE IF NOT EXISTS mail_ticket_ingest_log (
...
async function connectImap(
  ...
  return new Promise((resolve, reject) => {
    const onError = (error: any) => reject(new Error(error?.message || 'IMAP connection failed'))
    const onConnect = (sock: ImapSocket) => {
    ...
    if (secure) {
      const tlsSocket: tls.TLSSocket = tls.connect({ host, port, servername: host }, () => onConnect(tlsSocket))
      tlsSocket.once('error', onError)
      return
    }

    const plainSocket: net.Socket = net.connect({ host, port }, () => onConnect(plainSocket))
    plainSocket.once('error', onError)
  })
}

async function createImapSession(socket: ImapSocket) {
  let seq = 0
  let buffer = ''
  ...
  const onLine = (line: string) => {
    ...
    if (match) {
      const status = String(match[1] || 'BAD').toUpperCase() as 'OK' | 'NO' | 'BAD'
      const done = current
      current = null
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const wait = () => {
      ...
      if (Date.now() - started > 10000) return reject(new Error('IMAP greeting timeout'))
      setTimeout(wait, 40)
    }
    wait()
  })

  const run = (command: string): Promise<ImapCommandResult> => {
    if (current) return Promise.reject(new Error('IMAP command overlap'))
    seq += 1
    const tag = `A${seq}`
    return new Promise((resolve, reject) => {
...
function parseSearchResult(lines: string[]): string[] {
...
function parseFetchedHeaders(lines: string[]): ParsedMailHeader {
...
function extractTicketRefFromSubject(subject: string): string | null {
  ...
  const std = s.match(/\bSTD#\d{1,10}\b/i)
  if (std?.[0]) return std[0].toUpperCase()
...
async function alreadyProcessed(mailbox: string, uid: string): Promise<boolean> {
  const row = await queryOne<{ id: number }>(
    'SELECT id FROM mail_ticket_ingest_log WHERE mailbox = $1 AND uid = $2',
    [mailbox, uid]
  )
  return Boolean(row)
}
...
async function markProcessed(payload: {
  ...
  fromEmail: string
  subject: string
  ticketDbId: number | null
}) {
  await query(
    `INSERT INTO mail_ticket_ingest_log (mailbox, uid, message_id, from_email, subject, ticket_id)
     VALUES ($1, $2, $3, $4, $5, $6)
...
async function processMailboxOnce() {
  ...
  if (!pollEnabled) {
    logger.info('mail_ticket_job_skipped', { reason: 'disabled' })
    return
  }
  if (!cfg.imap.user || !cfg.imap.pass) {
    logger.warn('mail_ticket_job_skipped', { reason: 'imap_credentials_missing' })
    return
  }
  if (!mailbox) {
    logger.warn('mail_ticket_job_skipped', { reason: 'mailbox_not_configured' })
    return
  }
  ...
  try {
    ...
    if (loginResult.status !== 'OK') throw new Error('IMAP login failed')

    const selectResult = await session.run(`SELECT "${imapEscape(cfg.imap.mailbox)}"`)
    if (selectResult.status !== 'OK') throw new Error(`Cannot select mailbox ${cfg.imap.mailbox}`)

    const searchResult = await session.run('SEARCH UNSEEN')
    if (searchResult.status !== 'OK') throw new Error('IMAP search failed')
...
export function startMailToTicketJob() {
  const intervalMs = toInt(process.env.MAIL_TICKET_POLL_MS, 30000)
  const run = async () => {
    if (MAILBOX_LOCK.running) return
    MAILBOX_LOCK.running = true
    try {
      await processMailboxOnce()
    } catch (error: any) {
      logger.warn('mail_ticket_job_failed', { error: error?.message || String(error) })
    } finally {
  ...
  logger.info('mail_ticket_job_started', { intervalMs })
}
