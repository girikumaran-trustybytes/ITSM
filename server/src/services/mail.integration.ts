...
const DEFAULT_BASE_MAIL = String(
...
function createSmtpTransport(cfg: SmtpConfig, overrides?: SmtpTransportOverrides) {
...
function getSmtpTransportCandidates(cfg: SmtpConfig): Array<{ port: number; secure: boolean }> {
  const primary = { port: Number(cfg.port), secure: Boolean(cfg.secure) }
  const host = String(cfg.host || '').trim().toLowerCase()
  const canSwitchSubmissionPort = (
    host.includes('gmail.com') ||
    host.includes('outlook') ||
    host.includes('office365') ||
    host.includes('zoho')
  )
  if (!canSwitchSubmissionPort) return [primary]

  const candidates: Array<{ port: number; secure: boolean }> = [primary]
  if (primary.port === 465) candidates.push({ port: 587, secure: false })
  if (primary.port === 587) candidates.push({ port: 465, secure: true })
...
export async function sendSmtpMail(
  ...
  const sendWithTransport = async (smtpCfg: SmtpConfig, overrides?: SmtpTransportOverrides) => {
  ...
  const sendWithCandidate = async (smtpCfg: SmtpConfig, candidate: SmtpTransportOverrides) => {
    try {
  ...
  const sendWithCandidateFallbacks = async (smtpCfg: SmtpConfig) => {
    ...
    for (const candidate of candidates) {
      try {
        ...
        lastError = error
        if (!isTransientSmtpError(error) && !isTlsVersionMismatch(error)) break
      }
    }
  ...
  const shouldTrySeededFallback =
    isSmtpAuthError(lastError) ||
    isTransientSmtpError(lastError)

  if (!info && shouldTrySeededFallback) {
    const seededCfg: SmtpConfig = {
      host: 'smtp.gmail.com',
      port: 465,
      secure: true,
  ...
  if (!info) {
    ...
    const err: any = new Error(lastError?.message || 'SMTP delivery failed')
