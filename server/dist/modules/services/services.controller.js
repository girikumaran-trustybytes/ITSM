...
async function create(req, res) {
    try {
        ...
        await (0, logger_1.auditLog)({ action: 'create_service', entity: 'service', entityId: created.id, user: req.user?.id, meta: { name: created.name } });
...
async function update(req, res) {
    try {
        ...
        await (0, logger_1.auditLog)({ action: 'update_service', entity: 'service', entityId: updated.id, user: req.user?.id });
        res.json(updated);
    }
    catch (err) {
...
async function remove(req, res) {
    try {
        ...
        await (0, logger_1.auditLog)({ action: 'delete_service', entity: 'service', entityId: deleted.id, user: req.user?.id, meta: { name: deleted.name } });
