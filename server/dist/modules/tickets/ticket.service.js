...
async function ensureTicketTeamSchema() {
    if (!ticketTeamSchemaReady) {
        ticketTeamSchemaReady = (async () => {
            await (0, db_1.query)('ALTER TABLE "ticket" ADD COLUMN IF NOT EXISTS "teamId" TEXT');
            await (0, db_1.query)('ALTER TABLE "ticket" ALTER COLUMN "teamId" SET DEFAULT \'helpdesk\'');
            await (0, db_1.query)('CREATE INDEX IF NOT EXISTS idx_ticket_team_id ON "ticket"("teamId")');
            await (0, db_1.query)('CREATE INDEX IF NOT EXISTS idx_ticket_assignee_id ON "ticket"("assigneeId")');
        })();
    }
...
function estimateBase64DecodedSize(payload) {
...
async function getAgentTicketScope(userId) {
    ...
    try {
        const allQueues = await (0, db_1.query)('SELECT queue_key FROM ticket_queues');
...
async function buildTicketVisibilityClause(alias, params, viewer) {
    const role = (0, policy_1.normalizeRole)(viewer?.role);
    if (role === 'ADMIN')
        return '';
...
async function canAgentAccessTicket(ticket, userId) {
    if (!ticket)
        return false;
    if (Number(ticket.assigneeId || 0) === Number(userId))
        return true;
    await ensureTicketTeamSchema();
...
async function assertViewerTicketAccess(ticket, viewer, mode = 'view') {
    if (!ticket)
...
const getTickets = async (opts = {}, viewer) => {
    ...
    if (visibilityClause && visibilityClause !== '1=1')
    ...
    await (0, logger_1.auditLog)({
        ...
        user: viewer?.id || null,
        meta: {
            role: (0, policy_1.normalizeRole)(viewer?.role),
            page,
            pageSize,
            q: opts.q || '',
            total,
...
const getTicketById = async (id, viewer) => {
    ...
    await (0, logger_1.auditLog)({
        ...
        ticketId: t.ticketId,
        user: viewer?.id || null,
        meta: { role: (0, policy_1.normalizeRole)(viewer?.role), ticketDbId: t.id },
    });
    return t;
};
...
const addResponse = async (ticketId, opts) => {
    ...
    if (opts.sendEmail) {
        ...
        await mailer_service_1.default.sendTicketResponseStrict(targetEmail, t, opts.message, opts.subject, opts.cc, opts.bcc, attachmentRows.map((a) => ({
            filename: String(a.filename || `attachment-${a.id}`),
            path: String(a.path || ''),
            contentType: String(a.contentType || ''),
        })), String((opts.user && typeof opts.user === 'object'
            ? opts.user.name || opts.user.email
            : '') || '').trim() || undefined, opts.html, opts.text, String(t.category || ''));
    }
    return created;
};
...
const resolveTicketWithDetails = async (ticketId, opts) => {
    ...
    const changedById = await resolveChangedById(opts.user);
    await (0, db_1.query)('INSERT INTO "tickethistory" ("ticketId", "fromStatus", "toStatus", "changedById", "note", "internal", "createdAt") VALUES ($1, $2, $3, $4, $5, $6, NOW())', [t.id, from, 'Closed', changedById, opts.resolution || 'Ticket closed', false]);
...
const uploadTicketAttachments = async (ticketId, opts) => {
    ...
    const preparedFiles = files.map((file) => {
        ...
        const base64Payload = extractBase64Payload(String(file?.contentBase64 || ''));
        const estimatedSize = estimateBase64DecodedSize(base64Payload);
        if (estimatedSize === null) {
            throw { status: 400, message: `Attachment "${file?.name || 'file'}" has invalid base64 payload` };
        }
        if (estimatedSize > MAX_ATTACHMENT_SIZE_BYTES) {
            throw { status: 400, message: `Attachment "${file?.name || 'file'}" exceeds 32MB` };
        }
        if (estimatedSize !== declaredSize) {
    ...
    const totalEstimated = preparedFiles.reduce((sum, entry) => sum + entry.declaredSize, 0);
    if (totalEstimated > MAX_ATTACHMENT_BATCH_BYTES) {
        throw { status: 400, message: 'Total attachment size must be 32MB or less' };
    }
    const changedById = await resolveChangedById(opts.user);
    ...
    for (const entry of preparedFiles) {
        ...
        const declaredSize = entry.declaredSize;
        const binary = decodeBase64Payload(entry.base64Payload);
...
const updateTicket = async (ticketId, payload, user) => {
    ...
    if (!t)
        throw { status: 404, message: 'Ticket not found' };
    await assertViewerTicketAccess(t, user, 'update');
    const actorRole = (0, policy_1.normalizeRole)((user && typeof user === 'object' ? user.role : '') || '');
    const data = {};
    ...
    if (actorRole !== 'USER') {
        ...
        if (payload.teamId !== undefined)
            data.teamId = normalizeTeamId(payload.teamId);
    }
