...
const createTicket = async (req, res) => {
    try {
        ...
        if (!payload.createdFrom)
            payload.createdFrom = 'TB Asset Support Platform';
...
const respond = async (req, res) => {
    const id = req.params.id;
    const { message, sendEmail, to, cc, bcc, subject, attachmentIds, html, text } = req.validated?.body || req.body || {};
    if (!message || !message.trim())
        return res.status(400).json({ error: 'Message is required' });
    const user = req.user || 'system';
    try {
        const entry = await ticketService.addResponse(id, { message, user, sendEmail, to, cc, bcc, subject, attachmentIds, html, text });
        res.status(201).json(entry);
    }
    catch (err) {
