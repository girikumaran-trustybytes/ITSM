...
const secrets_1 = require("../../common/auth/secrets");
const ACCESS_EXPIRES = process.env.ACCESS_TOKEN_EXPIRES_IN || '15m';
const REFRESH_EXPIRES_DAYS = Number(process.env.REFRESH_TOKEN_EXPIRES_DAYS || 7);
const ACCESS_SECRET = (0, secrets_1.getRequiredSecret)('JWT_ACCESS_SECRET');
const REFRESH_SECRET = (0, secrets_1.getRequiredSecret)('JWT_REFRESH_SECRET');
...
// Avoid runtime DDL by default in production to prevent login outages when the
// app role is restricted to read/write and cannot ALTER/CREATE.
const AUTH_RUNTIME_SCHEMA_SYNC_ENABLED = toBool(process.env.AUTH_RUNTIME_SCHEMA_SYNC, process.env.NODE_ENV !== 'production');
...
async function ensureAuthSchema() {
    if (!AUTH_RUNTIME_SCHEMA_SYNC_ENABLED)
        return;
    if (!authSchemaInit) {
        ...
        ON CONFLICT ("id") DO NOTHING`);
          ...
          "createdAt" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
            ...
            await (0, db_1.query)('CREATE INDEX IF NOT EXISTS idx_user_email_lower ON "user"(LOWER("email"))');
            await (0, db_1.query)('CREATE INDEX IF NOT EXISTS idx_service_accounts_user_id ON "serviceaccounts"("userId")');
...
async function warmupAuthSchema() {
    await ensureAuthSchema();
}
exports.warmupAuthSchema = warmupAuthSchema;
async function warmupAuthDb() {
    await (0, db_1.queryOne)('SELECT 1');
    await ensureAuthSchema();
}
exports.warmupAuthDb = warmupAuthDb;
...
async function verifyUserPassword(inputPassword, storedHash) {
    const plain = String(inputPassword || '');
    const hash = String(storedHash || '');
    if (!plain || !hash)
        return false;
    try {
        if (await bcrypt_1.default.compare(plain, hash))
            return true;
    }
    catch {
        // Fall through to DB crypt() compatibility check.
    }
    try {
        const row = await (0, db_1.queryOne)('SELECT crypt($1, $2) = $2 AS ok', [plain, hash]);
...
async function login(email, password, trustedDeviceToken, rememberMe = false) {
    ...
    const ok = await verifyUserPassword(String(password || ''), user.password);
    if (!ok)
        throw new Error('Invalid credentials');
    const policy = await getMfaPolicy();
...
async function forgotPassword(email) {
    ...
    const resetBase = String(process.env.PASSWORD_RESET_BASE_URL ||
        process.env.RESET_PASSWORD_BASE_URL ||
        DEFAULT_PUBLIC_APP_URL).trim();
    const appBase = String(resetBase || FRONTEND_URL || DEFAULT_PUBLIC_APP_URL).replace(/\/+$/, '');
  ...
  `;
    try {
        ...
        // Do not leak account existence based on delivery behavior.
