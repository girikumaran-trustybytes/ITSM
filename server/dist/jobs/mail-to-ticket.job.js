...
async function appendInboundReplyToTicket(ticketDbId, mailbox, mail) {
    const ticket = await (0, db_1.queryOne)('SELECT "id", "ticketId", "status" FROM "ticket" WHERE "id" = $1', [ticketDbId]);
    if (!ticket?.id)
        return null;
    const message = [
    ...
    if (statusRaw && statusRaw !== 'closed' && statusRaw !== 'updated') {
        await (0, ticket_service_1.transitionTicket)(String(ticket.id), 'Updated', 'system');
    }
    return ticket;
}
