...
exports.acceptInvitationToken = exports.revokeInvitationById = exports.resendInvitationById = exports.inviteExistingUser = exports.createInvitationRequest = void 0;
...
const INVITE_RATE_WINDOW_MS = Math.max(60000, Number(process.env.INVITE_RATE_WINDOW_MS || 15 * 60 * 1000));
...
function hashInviteToken(rawToken) {
    return crypto_1.default.createHash('sha256').update(`${rawToken}:${TOKEN_PEPPER}`).digest('hex');
}
function choosePrimaryRole(roleNames) {
...
function invitationContext() {
    const appName = String(process.env.APPLICATION_NAME ||
        ...
        'TB ITSM').trim();
    const appBaseUrl = trimSlash(String(process.env.FRONTEND_URL ||
        process.env.APP_URL ||
...
function assertRateLimit(key, maxAttempts) {
    const now = Date.now();
...
async function ensureInvitationSchema() {
    if (!invitationSchemaReady) {
        invitationSchemaReady = (async () => {
            await (0, rbac_service_1.ensureRbacSeeded)();
            await (0, db_1.query)(`
      ...
      `);
            await (0, db_1.query)(`INSERT INTO tenants (id, name, status)
         ...
         ON CONFLICT (id) DO NOTHING`);
            ...
            await (0, db_1.query)(`CREATE INDEX IF NOT EXISTS idx_user_tenant_id ON "User"("tenantId")`);
            await (0, db_1.query)(`
      ...
      `);
            await (0, db_1.query)(`CREATE UNIQUE INDEX IF NOT EXISTS idx_invitations_token_hash_unique ON invitations(token_hash) WHERE token_hash IS NOT NULL`);
            await (0, db_1.query)(`CREATE INDEX IF NOT EXISTS idx_invitations_tenant_email_created ON invitations(tenant_id, email, created_at DESC)`);
            await (0, db_1.query)(`CREATE INDEX IF NOT EXISTS idx_invitations_user_created ON invitations(user_id, created_at DESC)`);
            await (0, db_1.query)(`
...
async function getUserRoleNames(userId, fallbackRole) {
    try {
        const rows = await (0, db_1.query)(`SELECT DISTINCT r.role_name
       FROM roles r
...
async function getUserPermissionSet(userId, roleNames) {
...
async function getActorContext(actorUserId) {
    ...
    const actor = await (0, db_1.queryOne)(`SELECT "id", "name", "email", "tenantId", "role"
     FROM "User"
...
async function resolveRoleRows(payload) {
    ...
    if (roleIdsInput.length > 0) {
        rows = await (0, db_1.query)(`SELECT role_id, role_name
       FROM roles
       WHERE role_id = ANY($1::int[])`, [roleIdsInput]);
    }
    if (rows.length === 0 && roleNamesInput.length > 0) {
        rows = await (0, db_1.query)(`SELECT role_id, role_name
       FROM roles
       WHERE UPPER(role_name) = ANY($1::text[])`, [roleNamesInput]);
    }
    if (rows.length === 0) {
        const fallback = await (0, db_1.query)(`SELECT role_id, role_name
       FROM roles
    ...
    if (rows.length === 0)
        throw { status: 400, message: 'No valid role found for invitation' };
    return rows.map((row) => ({ role_id: Number(row.role_id), role_name: normalizeRoleName(row.role_name) }));
}
async function getRolePermissionKeys(roleIds) {
    if (roleIds.length === 0)
        return new Set();
    try {
        const rows = await (0, db_1.query)(`SELECT DISTINCT p.permission_key
       FROM role_permissions rp
       INNER JOIN permissions p ON p.permission_id = rp.permission_id
       WHERE rp.role_id = ANY($1::int[])
         AND rp.allowed = TRUE`, [roleIds]);
...
async function assertAssignableByActor(actor, roleIds) {
    if (actor.permissions.has('*') || actor.roles.includes('ADMIN'))
        return;
...
async function getRoleRowsForUser(userId) {
    const rows = await (0, db_1.query)(`SELECT DISTINCT r.role_id, r.role_name
     FROM roles r
     INNER JOIN user_roles ur ON ur.role_id = r.role_id
     WHERE ur.user_id = $1`, [userId]);
    if (rows.length > 0) {
    ...
    const fallback = await (0, db_1.queryOne)(`SELECT r.role_id, r.role_name
     FROM roles r
    ...
    const userFallback = await (0, db_1.queryOne)(`SELECT role_id, role_name
     FROM roles
     WHERE role_name = 'USER'
     LIMIT 1`);
    if (!userFallback)
        throw { status: 500, message: 'RBAC roles are not initialized.' };
    return [{ role_id: Number(userFallback.role_id), role_name: 'USER' }];
}
async function getTeamKeysForUser(userId) {
    const serviceAccount = await (0, db_1.queryOne)(`SELECT "queueIds"
     FROM "ServiceAccounts"
     WHERE "userId" = $1
       AND "enabled" = TRUE`, [userId]);
    if (!serviceAccount)
        return [];
    if (!Array.isArray(serviceAccount.queueIds))
        return [];
...
async function syncTeamMembershipWithClient(client, userId, tenantId, teamKeys) {
    ...
    const teamRows = await client.query(`SELECT team_id, team_key
     FROM teams
     WHERE tenant_id = $1
...
async function syncUserRolesBestEffort(client, userId, roleIds) {
    const runBestEffort = async (sql, params, swallowErrorCodes) => {
        ...
        try {
            ...
            await client.query(`RELEASE SAVEPOINT ${sp}`);
            if (swallowErrorCodes.includes(String(error?.code || '')))
                return;
            throw error;
        }
    };
    try {
        await runBestEffort('DELETE FROM user_roles WHERE user_id = $1', [userId], ['42P01'] // table missing in legacy env
        );
    }
    catch (error) {
        if (error?.code === '42P01')
    ...
    for (const roleId of roleIds) {
        await runBestEffort(`INSERT INTO user_roles (user_id, role_id)
       VALUES ($1, $2)
       ON CONFLICT (user_id, role_id) DO NOTHING`, [userId, roleId], ['23503', '42P01'] // FK mismatch / table missing
        );
    }
}
async function applyRoleAndTeamAssignments(userId, tenantId, roleRows, teamKeys, status) {
    const roleIds = uniqueStrings(roleRows.map((row) => String(row.role_id))).map((value) => Number(value));
    const roleNames = roleRows.map((row) => normalizeRoleName(row.role_name));
    ...
    await (0, db_1.withClient)(async (client) => {
        ...
        try {
         ...
         SET "role" = $1,
             ...
             "tenantId" = $3,
             "updatedAt" = NOW()
         WHERE "id" = $4`, [primaryRole, status, tenantId, userId]);
            if (hasAgentRole) {
...
async function upsertInvitationMappings(invitationId, roleRows, teamKeys) {
    await (0, db_1.withClient)(async (client) => {
        await client.query('BEGIN');
        try {
            ...
            for (const roleRow of roleRows) {
                await client.query(`INSERT INTO invitation_roles (invitation_id, role_id)
           VALUES ($1, $2)
           ON CONFLICT (invitation_id, role_id) DO NOTHING`, [invitationId, roleRow.role_id]);
            }
            for (const teamKey of teamKeys) {
                await client.query(`INSERT INTO invitation_teams (invitation_id, team_key)
           VALUES ($1, $2)
           ON CONFLICT (invitation_id, team_key) DO NOTHING`, [invitationId, normalizeTeamKey(teamKey)]);
            }
            await client.query('COMMIT');
        }
        catch (error) {
...
async function ensureInvitedUser(email, tenantId, name, primaryRole) {
    const existing = await (0, db_1.queryOne)(`SELECT "id", "status", "tenantId", "name"
     FROM "User"
    ...
    if (existing) {
        ...
        if (name && String(existing.name || '').trim().length === 0) {
            await (0, db_1.query)(`UPDATE "User" SET "name" = $1, "updatedAt" = NOW() WHERE "id" = $2`, [name, existing.id]);
        }
        return { id: existing.id, status: existingStatus || 'INVITED' };
    }
    const randomPassword = crypto_1.default.randomBytes(32).toString('hex');
    const hashedPassword = await bcrypt_1.default.hash(randomPassword, 12);
    const created = await (0, db_1.queryOne)(`INSERT INTO "User" (
       "email", "password", "name", "role", "status", "tenantId", "createdAt", "updatedAt"
     )
     VALUES ($1, $2, $3, $4, 'INVITED', $5, NOW(), NOW())
     RETURNING "id"`, [email, hashedPassword, name, primaryRole, tenantId]);
    if (!created)
        throw { status: 500, message: 'Failed to create invited user.' };
    return { id: created.id, status: 'INVITED' };
}
async function createInvitationRow(input) {
    const row = await (0, db_1.queryOne)(`INSERT INTO invitations (tenant_id, user_id, email, invited_by, status, resend_count)
     VALUES ($1, $2, $3, $4, 'PENDING', 0)
     RETURNING invitation_id`, [input.tenantId, input.userId, input.email, input.invitedBy]);
    if (!row)
        throw { status: 500, message: 'Failed to create invitation.' };
    return Number(row.invitation_id);
}
async function sendInvitationEmail(email, name, inviteLink, expiresAt, mode, actor) {
    const ctx = invitationContext();
    const person = (String(name || '').trim() || 'User');
    const expiresTextIst = formatInviteExpiryIst(expiresAt);
    const subject = 'Invitation to Activate Your TB ITSM Account';
    const text = [
        `Dear ${person},`,
        '',
        ...
        '',
        `If you did not expect this invitation or require any assistance, please contact our support team at girikumaran@trustybytes.in.`,
        '',
        `Thank you,`,
        '',
        'TB Support Team',
    ...
    <div style="font-family:Arial,sans-serif;font-size:14px;line-height:1.55;color:#111827;background:#f3f4f6;padding:24px">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;padding:24px">
        ...
        <p style="margin:0 0 16px 0;color:#111827">If you did not expect this invitation or require any assistance, please contact our support team at <a href="mailto:girikumaran@trustybytes.in">girikumaran@trustybytes.in</a>.</p>
...
async function sendInviteFromInvitation(invitationId, actor, mode, incrementResendCount, meta) {
    const invitation = await (0, db_1.queryOne)(`SELECT
       i.invitation_id,
       i.user_id,
       i.email,
       i.status,
     ...
     WHERE i.invitation_id = $1`, [invitationId]);
    if (!invitation)
        throw { status: 404, message: 'Invitation not found.' };
    if (Number(invitation.tenant_id) !== actor.tenantId)
        throw { status: 403, message: 'Cross-tenant invitation access denied.' };
    ...
    await (0, db_1.query)(`UPDATE invitations
     ...
     WHERE invitation_id = $4`, [tokenHash, expiresAt, incrementResendCount, invitationId]);
    const ctx = invitationContext();
    ...
    const updated = await (0, db_1.queryOne)(`SELECT resend_count
     FROM invitations
...
async function createInvitationRequest(payload, actorUserId, meta) {
    ...
    if (payload.sendNow === false) {
        const existingPending = await (0, db_1.queryOne)(`SELECT invitation_id, last_sent_at
       FROM invitations
       WHERE tenant_id = $1
         AND user_id = $2
         AND status = 'PENDING'
       ORDER BY created_at DESC
       LIMIT 1`, [actor.tenantId, targetUser.id]);
        if (existingPending) {
            return {
    ...
    const latestPending = await (0, db_1.queryOne)(`SELECT invitation_id, last_sent_at
     FROM invitations
     WHERE tenant_id = $1
       AND user_id = $2
       AND status = 'PENDING'
...
async function inviteExistingUser(userId, actorUserId, options, meta) {
    await ensureInvitationSchema();
    ...
    const user = await (0, db_1.queryOne)(`SELECT "id", "email", "name", "tenantId", "status"
     FROM "User"
     WHERE "id" = $1`, [userId]);
    if (!user)
        throw { status: 404, message: 'User not found.' };
    const userTenantId = Number(user.tenantId || 1);
    if (userTenantId !== actor.tenantId)
        throw { status: 403, message: 'Cross-tenant invitation is not allowed.' };
    ...
    if (mode === 'reinvite') {
        const latest = await (0, db_1.queryOne)(`SELECT invitation_id, status
       FROM invitations
       WHERE tenant_id = $1
         AND user_id = $2
       ORDER BY created_at DESC
       LIMIT 1`, [actor.tenantId, userId]);
        if (!latest)
            throw { status: 404, message: 'No existing invitation found to re-invite.' };
...
async function resendInvitationById(invitationId, actorUserId, meta) {
    ...
    if (!Number.isFinite(invitationId) || invitationId <= 0)
        throw { status: 400, message: 'Invalid invitation id.' };
    const actor = await getActorContext(actorUserId);
    assertRateLimit(`invite:resend:${actor.userId}:${invitationId}`, INVITE_RESEND_RATE_LIMIT);
    const invitation = await (0, db_1.queryOne)(`SELECT invitation_id, status, tenant_id, user_id
     FROM invitations
     WHERE invitation_id = $1`, [invitationId]);
    if (!invitation)
        throw { status: 404, message: 'Invitation not found.' };
    ...
    if (Number(invitation.user_id || 0) > 0) {
        const user = await (0, db_1.queryOne)(`SELECT "status"
       FROM "User"
       WHERE "id" = $1`, [invitation.user_id]);
        if (String(user?.status || '').toUpperCase() === 'ACTIVE') {
            throw { status: 400, message: 'User account is already active. Use password reset flow.' };
        }
    }
...
async function revokeInvitationById(invitationId, actorUserId, meta) {
    await ensureInvitationSchema();
    if (!Number.isFinite(invitationId) || invitationId <= 0)
        throw { status: 400, message: 'Invalid invitation id.' };
    const actor = await getActorContext(actorUserId);
    const updated = await (0, db_1.queryOne)(`UPDATE invitations
     SET status = 'REVOKED',
         revoked_at = NOW(),
         token_hash = NULL,
         expires_at = NULL
     ...
     RETURNING invitation_id`, [invitationId, actor.tenantId]);
    if (!updated) {
        const existing = await (0, db_1.queryOne)(`SELECT status, tenant_id
       FROM invitations
       WHERE invitation_id = $1`, [invitationId]);
        if (!existing)
            throw { status: 404, message: 'Invitation not found.' };
        if (Number(existing.tenant_id) !== actor.tenantId)
            throw { status: 403, message: 'Cross-tenant invitation access denied.' };
...
async function acceptInvitationToken(token, password, name, meta) {
    await ensureInvitationSchema();
    ...
    const tokenHash = hashInviteToken(rawToken);
    const invitation = await (0, db_1.queryOne)(`SELECT invitation_id, tenant_id, user_id, email, status, expires_at
     FROM invitations
     WHERE token_hash = $1
     ORDER BY created_at DESC
     LIMIT 1`, [tokenHash]);
    if (!invitation)
        throw { status: 400, message: 'Invitation token is invalid.' };
    const status = String(invitation.status || '').toUpperCase();
    if (status === 'REVOKED')
        throw { status: 400, message: 'Invitation has been revoked.' };
    if (status === 'ACCEPTED')
        throw { status: 400, message: 'Invitation has already been used.' };
    if (status !== 'PENDING')
        throw { status: 400, message: `Invitation is not in a valid state: ${status}` };
    const expiresAtMs = invitation.expires_at ? new Date(invitation.expires_at).getTime() : 0;
    if (!Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()) {
        await (0, db_1.query)(`UPDATE invitations
       SET status = 'EXPIRED',
           token_hash = NULL
       WHERE invitation_id = $1`, [invitation.invitation_id]);
        throw { status: 400, message: 'Invitation has expired. Ask your administrator to re-invite you.' };
    }
    const roleRows = await (0, db_1.query)(`SELECT r.role_id, r.role_name
     FROM invitation_roles ir
     INNER JOIN roles r ON r.role_id = ir.role_id
     WHERE ir.invitation_id = $1`, [invitation.invitation_id]);
    ...
    if (effectiveRoleRows.length === 0) {
        const fallbackRole = await (0, db_1.queryOne)(`SELECT role_id, role_name
       FROM roles
       WHERE role_name = 'USER'
       LIMIT 1`);
        if (!fallbackRole)
    ...
    const teamRows = await (0, db_1.query)(`SELECT team_key
    ...
    await (0, db_1.withClient)(async (client) => {
        await client.query('BEGIN');
        try {
            const lockedInvite = await client.query(`SELECT invitation_id, tenant_id, user_id, email, status, expires_at
         FROM invitations
         WHERE invitation_id = $1
         FOR UPDATE`, [invitation.invitation_id]);
            if (lockedInvite.rows.length === 0)
                throw { status: 404, message: 'Invitation not found.' };
            const row = lockedInvite.rows[0];
            const lockedStatus = String(row.status || '').toUpperCase();
            if (lockedStatus !== 'PENDING')
                throw { status: 400, message: `Invitation is not in a valid state: ${lockedStatus}` };
           ...
           SET status = 'EXPIRED',
               token_hash = NULL
           WHERE invitation_id = $1`, [row.invitation_id]);
                throw { status: 400, message: 'Invitation has expired. Ask your administrator to re-invite you.' };
            }
            ...
            let userStatus = '';
            if (targetUserId > 0) {
                const byId = await client.query(`SELECT "id", "status", "tenantId"
           FROM "User"
           WHERE "id" = $1
           FOR UPDATE`, [targetUserId]);
            ...
            if (targetUserId <= 0) {
                const byEmail = await client.query(`SELECT "id", "status", "tenantId"
           FROM "User"
           WHERE LOWER("email") = LOWER($1)
           LIMIT 1
           FOR UPDATE`, [normalizedEmail]);
            ...
            if (targetUserId > 0 && userStatus === 'ACTIVE') {
                throw { status: 400, message: 'User account is already active. Use password reset flow.' };
            }
            if (targetUserId > 0) {
                await client.query(`UPDATE "User"
           SET "password" = $1,
               ...
               "tenantId" = $4,
               "updatedAt" = NOW()
           WHERE "id" = $5`, [passwordHash, nextName || '', primaryRole, tenantId, targetUserId]);
            }
            else {
                const inserted = await client.query(`INSERT INTO "User" ("email", "password", "name", "role", "status", "tenantId", "createdAt", "updatedAt")
           VALUES ($1, $2, $3, $4, 'ACTIVE', $5, NOW(), NOW())
           RETURNING "id"`, [normalizedEmail, passwordHash, nextName || normalizedEmail, primaryRole, tenantId]);
           ...
           DO UPDATE SET
             ...
             "autoUpgradeQueues" = FALSE,
             "queueIds" = EXCLUDED."queueIds",
             "updatedAt" = NOW()`, [targetUserId, teamKeys]);
         ...
         SET user_id = $1,
             ...
             token_hash = NULL,
             expires_at = NULL
         WHERE invitation_id = $2`, [targetUserId, row.invitation_id]);
            await client.query(`UPDATE invitations
         SET status = 'REVOKED',
             revoked_at = NOW(),
             token_hash = NULL,
         ...
         WHERE tenant_id = $1
           ...
           AND invitation_id <> $3
           AND status = 'PENDING'`, [tenantId, normalizedEmail, row.invitation_id]);
