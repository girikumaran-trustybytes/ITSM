...
const svc = __importStar(require("./sla.service"));
...
async function create(req, res) {
    try {
        ...
        await (0, logger_1.auditLog)({ action: 'create_sla', entity: 'sla', entityId: created.id, user: userId, meta: { name: created.name } });
...
async function update(req, res) {
    try {
        ...
        const userId = req.user?.id;
        await (0, logger_1.auditLog)({ action: 'update_sla', entity: 'sla', entityId: updated.id, user: userId });
        res.json(updated);
    }
    catch (err) {
...
async function remove(req, res) {
    try {
        ...
        const userId = req.user?.id;
        await (0, logger_1.auditLog)({ action: 'delete_sla', entity: 'sla', entityId: deleted.id, user: userId, meta: { name: deleted.name } });
