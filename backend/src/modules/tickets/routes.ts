...
router.get('/', validate({ query: ticketsListQuerySchema }), ctrl.listTickets)
router.get('/:id', validate({ params: ticketIdParamsSchema }), ctrl.getTicket)
router.post('/', permit(['ADMIN','AGENT','USER']), validate({ body: ticketsCreateBodySchema }), ctrl.createTicket)
router.post('/:id/transition', permit(['ADMIN','AGENT']), validate({ params: ticketIdParamsSchema, body: ticketsTransitionBodySchema }), ctrl.transitionTicket)
router.post('/:id/mark-responded', permit(['ADMIN','AGENT']), validate({ params: ticketIdParamsSchema }), ctrl.markResponded)
...
router.patch('/:id', permit(['ADMIN','AGENT']), validate({ params: ticketIdParamsSchema, body: ticketsUpdateBodySchema }), ctrl.updateTicket)
router.delete('/:id', permit(['ADMIN']), validate({ params: ticketIdParamsSchema }), ctrl.deleteTicket)
router.get('/:id/audit', permit(['ADMIN','AGENT']), validate({ params: ticketIdParamsSchema }), (req, res) => import('./audit.controller').then(m => m.getAudit(req, res)))
