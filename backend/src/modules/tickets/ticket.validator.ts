...
export function validateUpdate(payload: any) {
  if (!payload) return { ok: false, message: 'Missing body' }
  // allow partial updates but ensure types are correct when present
  if (payload.subject && typeof payload.subject !== 'string') return { ok: false, message: 'Invalid subject' }
  if (payload.summary && typeof payload.summary !== 'string') return { ok: false, message: 'Invalid subject' }
  if (payload.type && typeof payload.type !== 'string') return { ok: false, message: 'Invalid type' }
  return { ok: true }
}
