...
function isEmailLike(value: unknown): string {
  const text = String(value || '').trim()
  if (!text) return ''
  return text.includes('@') ? text : ''
}

function normalizeMailboxProvider(value: unknown, smtpHost?: string, imapHost?: string): 'gmail' | 'outlook' | 'zoho' | 'custom' {
  const raw = String(value || '').trim().toLowerCase()
  if (raw === 'gmail' || raw === 'google' || raw === 'google-workspace') return 'gmail'
  if (raw === 'outlook' || raw === 'microsoft-workspace' || raw === 'office365' || raw === 'microsoft-365') return 'outlook'
  if (raw === 'zoho') return 'zoho'
  if (raw === 'custom') return 'custom'
  const hostBlob = `${smtpHost || ''} ${imapHost || ''}`.toLowerCase()
  if (hostBlob.includes('gmail')) return 'gmail'
  if (hostBlob.includes('zoho')) return 'zoho'
  if (hostBlob.includes('outlook') || hostBlob.includes('office365')) return 'outlook'
  return 'custom'
}

function buildRoutingFromMailboxes(mailboxes: Array<any>, fallbackQueue: string) {
  const inboundRoutes: Array<{ email: string; queue: string }> = []
  const outboundRoutes: Array<{ queue: string; from: string }> = []
...
function logMailMigration(event: string, details?: Record<string, any>) {
  const payload = details ? ` ${JSON.stringify(details)}` : ''
  console.info(`[mail-settings] ${event}${payload}`)
}

function buildOauthStartUrl(provider: string, mailboxId: string) {
  const safeMailboxId = String(mailboxId || '').trim()
  if (!safeMailboxId) throw { status: 400, message: 'mailboxId is required' }
  if (provider === 'gmail') {
    ...
    if (!clientId || !redirectUri) throw { status: 400, message: 'Google OAuth is not configured' }
  ...
  if (provider === 'outlook') {
    ...
    const scope = String(
      process.env.MAIL_OUTLOOK_SCOPES
      || 'https://outlook.office.com/IMAP.AccessAsUser.All https://outlook.office.com/SMTP.Send offline_access'
    ).trim()
    if (!clientId || !redirectUri) throw { status: 400, message: 'Outlook OAuth is not configured' }
    const params = new URLSearchParams({
      client_id: clientId,
      redirect_uri: redirectUri,
      response_type: 'code',
  ...
  if (provider === 'zoho') {
    ...
    const accountsBase = String(process.env.MAIL_ZOHO_ACCOUNTS_BASE || 'https://accounts.zoho.com')
      .trim()
      .replace(/\/+$/, '')
    if (!clientId || !redirectUri) throw { status: 400, message: 'Zoho OAuth is not configured' }
    const params = new URLSearchParams({
      client_id: clientId,
      redirect_uri: redirectUri,
      response_type: 'code',
...
function buildLegacyMailbox(normalized: StoredMailSettings, envSupport: string): any | null {
  const smtp = normalized.smtp || {}
  const imap = normalized.imap || {}
  const hasLegacyConfig = Boolean(
    smtp.host || smtp.user || smtp.pass || imap.host || imap.user || imap.pass
  )
  if (!hasLegacyConfig) return null
  const fallbackQueue = String(normalized.inbound?.defaultQueue || 'Support Team').trim() || 'Support Team'
  const candidates = [
    envSupport,
    (normalized.settings as any)?.supportMail,
    (normalized.settings as any)?.inboundEmailAddress,
    smtp.from,
    smtp.user,
    imap.user,
...
function normalizeStoredMailSettings(input: any): StoredMailSettings {
  ...
  const mailboxesRaw = Array.isArray(raw.mailboxes)
    ? raw.mailboxes
    : (Array.isArray((settings as any).mailboxes) ? (settings as any).mailboxes : [])
  ...
  const mailboxes = Array.isArray(mailboxesRaw)
    ? mailboxesRaw
      .map((row: any) => {
        ...
        const connectionMode =
          connectionModeRaw === 'oauth2' ? 'oauth2'
...
function assertUniqueMailboxes(mailboxes: Array<any>) {
...
function dedupeMailboxes(mailboxes: Array<any>) {
  const byEmail = new Map<string, any>()
  const order: string[] = []
  const pickString = (a: any, b: any) => (String(a || '').trim() ? a : b)
  const pickBool = (a: any, b: any) => (typeof a === 'boolean' ? a : (typeof b === 'boolean' ? b : undefined))
  const mergeMailbox = (base: any, extra: any) => {
    const mergedOauth = {
      ...(base?.oauth || {}),
      ...(extra?.oauth || {}),
...
export async function getConfig(_req: Request, res: Response) {
  try {
    ...
    if (stored) {
      ...
      if (envSmtpUser) normalized.smtp = { ...normalized.smtp, user: envSmtpUser }
      if (envImapUser) normalized.imap = { ...normalized.imap, user: envImapUser }
      if (envFrom) normalized.smtp = { ...normalized.smtp, from: envFrom }
      ...
      if (envSupport && Array.isArray(normalized.mailboxes)) {
        ...
        const hasSupport = normalized.mailboxes.some((row: any) => String(row.email || '').trim().toLowerCase() === supportLower)
        if (!hasSupport) {
      ...
      if (envSupport) {
        ...
        const nextInbound = hasSupportEmailInbound
          ...
          : inboundRoutes.map((row) => (
            String(row.queue || '').trim().toLowerCase() === 'support team'
              ? { ...row, email: supportLower }
              : row
          ))
        const nextOutbound = hasSupportEmailOutbound
          ? outboundRoutes
          : outboundRoutes.map((row) => (
            String(row.queue || '').trim().toLowerCase() === 'support team'
        ...
        if (!hasSupportEmailInbound && !hasSupportQueueInbound) nextInbound.push({ email: supportLower, queue: 'Support Team' })
        if (!hasSupportEmailOutbound && !hasSupportQueueOutbound) nextOutbound.push({ queue: 'Support Team', from: supportLower })
        normalized.inbound = {
          ...(normalized.inbound || {}),
  ...
  try {
    ...
    if (Array.isArray(incoming.mailboxes) && incoming.mailboxes.length === 0) {
      ...
      if (existingMailboxes.length) {
        ...
        const mergedLegacy: StoredMailSettings = {
          ...storedNormalized,
          ...incoming,
    ...
    if (!migratedLegacyMailbox && Array.isArray(incoming.mailboxes) && incoming.mailboxes.length) {
      const supportCandidate = String(
        incoming.settings?.supportMail
    ...
    if (Array.isArray(incoming.mailboxes)) {
      ...
      const findStored = (row: any) => {
        const id = String(row?.id || '').trim()
        if (id) return storedMailboxes.find((s: any) => String(s?.id || '').trim() === id)
        const email = String(row?.email || '').trim().toLowerCase()
        if (!email) return undefined
        return storedMailboxes.find((s: any) => String(s?.email || '').trim().toLowerCase() === email)
      }
      const mergedMailboxes = incoming.mailboxes.map((row: any) => {
      ...
      setMailboxConfigs(mergedMailboxes as any)
    }
    const override: Partial<MailConfig> = {}
    ...
    const sanitizedMailboxes = Array.isArray(incoming.mailboxes)
      ? incoming.mailboxes.map((row: any) => ({
        id: row.id,
        email: row.email,
        queue: row.queue,
        provider: row.provider,
