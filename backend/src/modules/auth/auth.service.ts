...
function normalizeRole(input: string | null | undefined) {
...
async function getPrimaryRole(user: AuthUser) {
  ...
  try {
    const roleRows = await query<{ role_name: string }>(
      'SELECT r.role_name FROM roles r INNER JOIN user_roles ur ON r.role_id = ur.role_id WHERE ur.user_id = $1',
      [user.id]
    )
    const ranked = roleRows
      .map((row) => normalizeRole(row.role_name))
      .filter((value, index, list) => value && list.indexOf(value) === index)
      .sort((a, b) => rolePriority(a) - rolePriority(b))
    if (ranked.length > 0) {
      const preferred = ranked[0]
      return rolePriority(preferred) <= rolePriority(fallbackRole) ? preferred : fallbackRole
    }
  } catch {
